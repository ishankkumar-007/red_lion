...
char* crc_generator(char* data, char* divisor)
{
    ...
    for (int i=0; i < data_len; i++)
    {
        if (appended_data[i] == '1') 
        {
            ...
            {
                appended_data[i + j] = xorr(appended_data[i + j], divisor[j]);
            }
        }
    }
...
void crc_checker(char* data, char* divisor)
{
    ...
    for (int i=0; i < data_len; i++)
    {
        if (data[i] == '1') 
        {
            for (int j = 0; j < divisor_len; j++) 
            {
                data[i + j] = xorr(data[i + j], divisor[j]);
            }
        }
    }
...
int main()
{
    ...
    printf("\nEnter divisor: ");
    // + 1 to store \n
    fgets(divisor, sizeof(divisor)+1, stdin);    // if second argument is 9 it will only store < 9 chars
